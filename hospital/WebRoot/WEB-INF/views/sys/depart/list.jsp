                                      ...
                                      <c:forEach items="${depart.child}" var="a">
		                                <tr data-tt-id="${a.id}" data-tt-parent-id="${a.parent_id}" style="height: 44px;">
		                                    ...
		                                    <td>
					                            ...
					                            <c:if test="${fn:length(a.child)<=0}">
						                            <shiro:hasPermission name="manage:depart:del">
			                                    	<a class="btn btn-xs btn-white" onclick="del('${a.id}')"><i class="fa fa-trash"></i> 删除</a>
			                                    	</shiro:hasPermission>
		                                    	</c:if>
		                                    </td>
		                                </tr>
		                                 ...
		                                 <c:forEach items="${a.child }" var="b">
				                                <tr data-tt-id="${b.id}" data-tt-parent-id="${b.parent_id}" style="height: 44px;">
				                                    ...
				                                    <td>
							                            ...
							                            <c:if test="${fn:length(b.child)<=0}">
								                            <shiro:hasPermission name="manage:depart:del">
					                                    	<a class="btn btn-xs btn-white" onclick="del('${b.id}')"><i class="fa fa-trash"></i> 删除</a>
					                                    	</shiro:hasPermission>
					                                    </c:if>
				                                    </td>
				                                </tr>
				                                ...
				                                <c:forEach items="${b.child }" var="c">
					                                <tr data-tt-id="${c.id}" data-tt-parent-id="${c.parent_id}" style="height: 44px;">
					                                    ...
					                                    <td>
								                            ...
								                            <c:if test="${fn:length(c.child)<=0}">
									                            <shiro:hasPermission name="manage:depart:del">
						                                    	<a class="btn btn-xs btn-white" onclick="del('${c.id}')"><i class="fa fa-trash"></i> 删除</a>
						                                    	</shiro:hasPermission>
					                                    	</c:if>
					                                    </td>
					                                </tr>
					                                ...
					                                <c:forEach items="${c.child }" var="d">
						                                <tr data-tt-id="${d.id}" data-tt-parent-id="${d.parent_id}" style="height: 44px;">
						                                    ...
						                                    <td>
									                            ...
									                            <c:if test="${fn:length(d.child)<=0}">
										                            <shiro:hasPermission name="manage:depart:del">
							                                    	<a class="btn btn-xs btn-white" onclick="del('${d.id}')"><i class="fa fa-trash"></i> 删除</a>
							                                    	</shiro:hasPermission>
						                                    	</c:if>
						                                    </td>
						                                </tr>
						                                ...
						                                <c:forEach items="${d.child }" var="e">
							                                <tr data-tt-id="${e.id}" data-tt-parent-id="${e.parent_id}" style="height: 44px;">
							                                    ...
							                                    <td>
							                                    	...
							                                    	<shiro:hasPermission name="manage:depart:del">
							                                    	<a class="btn btn-xs btn-white" onclick="del('${e.id}')"><i class="fa fa-trash"></i> 删除</a>
							                                    	</shiro:hasPermission>
							                                    </td>
							                                </tr>
		                                               </c:forEach><!-- 6 -->
    ...
    <script>
    	...
    	function del(id){
        	layer.confirm('确定要操作吗？', function(index){
			  layer.close(index);
			  $.post("${ctx}/manage/depart/del/"+id,function(data){
				if(data.code=="0"){
					opt_success("操作成功");
				}else if(data.code=="-1"){
					opt_error("该部门有子部门，不能删除");
				}else if(data.code=="-2"){
				   opt_error("该部门有会员存在，不能删除");
				}else{
