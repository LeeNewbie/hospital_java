                    ...
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content">
                        	<div class="ibox-tools" style="float:left;">
                        		<shiro:hasPermission name="manage:hlabel:add">
                                <a class="btn btn-success btn-outline" onclick="layer_show('新增标签','${ctx}/manage/hlabel/add/0','','','')"><i class="fa fa-plus"></i> 新增一级标签</a>
                                <a class="btn btn-success btn-outline" onclick="layer_show('新增标签','${ctx}/manage/hlabel/add/-1','','','')"><i class="fa fa-plus"></i> 新增二级标签</a>
                                </shiro:hasPermission>
                               
                            </div>
							<form action="${ctx }/manage/hlabel/list" method="post" id="searchForm" class="pull-right mail-search" target='_self'>
		                        <div class="input-group">
		                            <input type="text" class="form-control input-sm" name="keywords" value="${vo.keywords}" placeholder="名称">
                            ...
                            <table class="footable table table-stripped toggle-arrow-tiny" data-sort="false" data-page-size="${vo.pageSize }">
                                ...
                                <tbody>
                                <c:forEach items="${page.result }" var="hlabel">
                                <tr>
                                	<td><input type="checkbox" class="i-checks" name="input[]" value="${hlabel.id }"></td>
                                    <c:if test="${hlabel.pid=='0'}">
                                     <td>${hlabel.name }</td>
                                     <td></td>
                                    </c:if>
                                    <c:if test="${hlabel.pid!='0'}">
                                    ...
                                    <td>
                                    	<shiro:hasPermission name="manage:hlabel:update">
                                    	<a class="btn btn-xs btn-white" onclick="layer_show('修改','${ctx}/manage/hlabel/update/${hlabel.id }','','','')"><i class="fa fa-pencil"></i> 修改</a>
                                    	</shiro:hasPermission>
                                    </td>
                                </tr>
                                </c:forEach>
