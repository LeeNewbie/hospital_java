...
//获取两个日期相差小时数
function getIntervalHours(d1, d2) {
	d1 = new Date(d1.replace(/-/g, "/"));
	d2 = new Date(d2.replace(/-/g, "/"));
	var hours=(d2.getTime()-d1.getTime())*1.0/(1000 * 60 * 60);
	return hours>parseInt(hours)?parseInt(hours)+1:parseInt(hours);
}
//获取经纬度之间距离
function Rad(d) {
